...
if (!cmsms()) exit;
	...
	/*---------------------------------------------------------
	  ...
	  ---------------------------------------------------------*/
		...
		$sqlarray = $dict->CreateTableSQL(cms_db_prefix()."module_twitter",
				$flds, $taboptarray);
		$dict->ExecuteSQLArray($sqlarray);

		// create a sequence
		$db->CreateSequence(cms_db_prefix()."module_twitter_seq");
		*/
		
		// permissions
		$this->CreatePermission('Manage Twitter','Manage Twitter');

		// Preferences
	//	$this->SetPreference('login','');
	//	$this->SetPreference('password','');
		$this->SetPreference('timeout','600');
		
//		$this->SetPreference('request')

		// put mention into the admin log
		$this->Audit( 0, $this->Lang('friendlyname'), $this->Lang('installed',$this->GetVersion()));
		
		// Template
	    $this->SetTemplate('timeline', $this->GetTemplateFromFile('timeline'));
